...
function MyJobs() {
    ...
    useEffect(() => {
        setIsLoading(true);
        fetch(`http://localhost:5000/myJobs/ranga@gmail.com`).then(res => res.json()).then(data => {
            setJobs(data);
            setIsLoading(false);
        })
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
        <div className='my-jobs-container'>
            ...
            <div className='search-box p-2 text-center mb-2'><input onChange={e => setSearchText(e.target.value)} type="text" name='search' id='search' className='py-2 pl-3 border focus:outline-none lg:w-6/12 mb-4 w-full' />
            <button className='bg-blue text-white font-semibold px-8 py-2 rounded-sm mb-4 ' onClick={handleSearch}>Search</button></div>
        </div>
        {/* table */}
